...
import { db, pgp } from '../../../db/postresql';
import Utils from './utils';
import MemesEngine from '../MemesEngine';

test('[MemesEngine] add/remove mem', async () => {
    await Utils.createUser(
        async (args) =>
            await Utils.generateRandomTags(
                2,
                async (args) => {
                    ...
                    const mem_id = await MemesEngine.addMem(memTemplate);
                    const mem_tags = await db.memes.memesTagsQueries.getMemTags(
                        { mem_id }
                    );
                    expect(mem_tags).toMatchObject(tags);

                    const userWatchedMemes = await db.users.usersWatchedMemesQueries.getUserWatchedMemIds(
                        { user_id }
                    );
                    expect(userWatchedMemes.length).toBe(1);
                    expect(userWatchedMemes[0]).toBe(mem_id);

                    await MemesEngine.removeMem({ mem_id });
                    const deletedMem = await db.memes.memesBaseQueries.getMem({
                        mem_id,
                    });
                    expect(deletedMem).toBeNull();
                },
                args
            )
    );
...
test('[MemesEngine] Two users, each add mem, first like mem of second, second dislike mem of first', async () => {
