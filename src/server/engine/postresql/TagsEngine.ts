import ITagsEngine, {AddTags, RateTags, RemoveTag} from "../IEngine/ITagsEngine";
...
const TagsEngine: ITagsEngine = class {
    static addTags(req: AddTags): Promise<void> {
        return db.tx( async (transaction) => {
            for(let i = 0; i < req.tags.length; ++i) {
                await transaction.tags.tagsBaseQueries.addTag({tag: req.tags[i]});
            }
        });
    }
    static rateTags(req: RateTags): Promise<void> {
        return db.tx( async (transaction) => {
            ...
            for(let i = 0; i < req.tags.length; ++i) {
                await transaction.tags.tagsBaseQueries.updateTagRating({tag: req.tags[i], like });
            }
        });
    }
    static removeTag(req: RemoveTag): Promise<void>{
        return db.tx( async (transaction) => {
            ...
            await transaction.tags.tagsBaseQueries.removeTagFromUsersTagsRating(req);
