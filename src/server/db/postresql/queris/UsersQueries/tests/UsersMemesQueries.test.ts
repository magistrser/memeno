...
test('[UsersMemesQueries] addUserMem/getUserMemIds/removeFromUsersMemes', async () => {
    ...
    const mem_id_1 = await db.memes.memesBaseQueries.addMem({
        mem_data: Buffer.from(base64String, 'base64'),
        user_id,
    });
    const mem_id_2 = await db.memes.memesBaseQueries.addMem({
        mem_data: Buffer.from(base64String, 'base64'),
        user_id,
    });

    await db.users.usersMemesQueries.addUserMem({ user_id, mem_id: mem_id_1 });
    await db.users.usersMemesQueries.addUserMem({ user_id, mem_id: mem_id_2 });

    const userMemes = await db.users.usersMemesQueries.getUserMemIds({user_id});
    expect(userMemes.length).toBe(2);
    expect(userMemes[0]).toBe(mem_id_1);
    expect(userMemes[1]).toBe(mem_id_2);

    await db.users.usersMemesQueries.removeFromUsersMemes({user_id});
    const deletedUserMemes = await db.users.usersMemesQueries.getUserMemIds({user_id});
    expect(deletedUserMemes.length).toBe(0);

    await db.memes.memesBaseQueries.removeMem({ mem_id: mem_id_1 });
    await db.memes.memesBaseQueries.removeMem({ mem_id: mem_id_2 });
