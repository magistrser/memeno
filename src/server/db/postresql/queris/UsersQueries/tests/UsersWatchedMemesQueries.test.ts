...
test('[UsersWatchedMemesQueries] addUserWatchedMem/getUserWatchedMemIds/removeFromUsersWatchedMemes', async () => {
    ...
    const mem_id1 = await db.memes.memesBaseQueries.addMem({
        mem_data: Buffer.from(base64String, 'base64'),
        user_id,
    });
    const mem_id2 = await db.memes.memesBaseQueries.addMem({
        mem_data: Buffer.from(base64String, 'base64'),
        user_id,
    });
    ...
    await db.memes.memesBaseQueries.removeMem({ mem_id: mem_id1 });
    await db.memes.memesBaseQueries.removeMem({ mem_id: mem_id2 });
